...
class _Page3State extends State<Page3> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    mainTimeFadeInAnimation = Tween(begin: 0.0, end: 1.0)
        .animate(CurvedAnimation(parent: controller, curve: const Interval(0.5, 1.0, curve: Curves.ease)));
    mainBodySlideUpAnimation = Tween<Offset>(begin: const Offset(0, 0), end: const Offset(0, -0.1))
        .animate(CurvedAnimation(parent: controller, curve: const Interval(0.5, 1.0, curve: Curves.ease)));
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: Column(
        ...
        children: [
          SizedBox(height: 40, child: Text(sampleContentList[index]["content"]!)),
          const SizedBox(height: 20, child: Text("See more ", style: TextStyle(fontWeight: FontWeight.bold))),
        ],
      ),
    );
  }
...
class _BodyMainInfoWidgetState extends State<BodyMainInfoWidget> {
  ...
  void initState() {
    ...
    calendarScaleUpAnimation = Tween(begin: 0.0, end: 1.0)
        .animate(CurvedAnimation(parent: controller, curve: const Interval(0.0, 0.7, curve: Curves.ease)));
    //for scale, fade in
    gpsIconScaleUpFadeInAnimation = Tween(begin: 0.0, end: 1.0)
        .animate(CurvedAnimation(parent: controller, curve: const Interval(0.25, 0.7, curve: Curves.ease)));
    //for scale, fade in
    timerScaleUpFadeInAnimation = Tween(begin: 0.0, end: 1.0)
        .animate(CurvedAnimation(parent: controller, curve: const Interval(0.5, 1.0, curve: Curves.ease)));
  }
              ...
              children: [
                ...
                Row(
                  children: [
                    ...
                    FadeTransition(
                        ...
                        child: Text(sampleContentList[widget.index]["location"]!))
                  ],
                ),
                const SizedBox(
                  height: 10,
                ...
                Row(
                  ...
                  children: [
                    ...
                    const SizedBox(width: 5),
                    FadeTransition(opacity: timerScaleUpFadeInAnimation, child: const Text("8:00 AM - 10:00 PM"))
                  ],
                ),
              ],
            ),
...
class HeaderAppBar extends StatefulWidget implements PreferredSizeWidget {
  const HeaderAppBar({super.key, required this.index, required this.controller});
...
class _HeaderAppBarState extends State<HeaderAppBar> {
  ...
  @override
  void initState() {
    headerBodySlideAnimation = Tween<Offset>(begin: const Offset(0, 0), end: const Offset(0, -0.1))
        .animate(CurvedAnimation(parent: widget.controller, curve: const Interval(0.5, 1.0, curve: Curves.ease)));
